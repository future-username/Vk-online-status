...
while True:
    try:
        ...
        now_date_ege, ege_date = now_date.strftime(format_date_ege).split('.'), date(int(2022), int(6), int(20))    #тут поменяй дату
...
● Или {hours} часов {minutes} минут"""})    
        ...
        sleep(30)ken=vk_token).captcha_handler(captcha)
        sleep(30)
